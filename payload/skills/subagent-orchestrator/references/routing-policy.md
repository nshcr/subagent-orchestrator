...
Use the smallest route that can finish the user outcome. Every child is a leaf;
the primary remains the only coordinator.

## Eligibility

| Need | Route | Admission test |
|---|---|---|
...
- Gather a complete review batch before repair. Perform at most one autonomous
  ...
  owner or action.
- A reviewer finding outside the named invariants is a deferred observation, not a
...
- Do not replace or interrupt a running child for silence or a wait timeout. A proven
  stale state, explicit cancellation, scope violation, or terminal platform failure
  is different.

`risk_reviewer_max` is available only after one `risk_reviewer` returns an explicitly
indeterminate high-risk decision with sufficient competing evidence. It is never a
