...
对操作叶子（`explorer`、`worker`、`evidence_tester`、`boundary_mapper`），主代理在原范围内最多进行一次有界更新。仅补充新纳入的证据、缺失的验收字段或新的失败证据。不得更新 `risk_reviewer` 或 `risk_reviewer_max`；审查证据变化必须重新冻结并启动新审查。不得反复查询子代理状态、要求重设计或扩大范围。
